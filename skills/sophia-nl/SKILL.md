...
metadata:
  hermes:
    ...
    requires_tools: [luvia_setup, luvia_set_method, luvia_plan_today, luvia_pick_items, luvia_score_response, luvia_record_result, luvia_stats, memory, text_to_speech, send_message, todo]
    related_skills: []
---
...
- **Luvia link** — a `Sophia luvia:` block: `user_id` (from `luvia_setup`), `target_lang`
  (nl), `last_plan_date`, today's `band`/`new_intake`/`due_load` from the daily plan, and a
  short note on how much Dutch they can currently handle. This is how she resumes teaching
  without re-linking.
...
- [ ] `/wife` captures `owner_target`, single-use `pair_code`, first-person relay to
      `partner_target`; relay never revealed to the owner.

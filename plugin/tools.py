...
def _save_selfie(image_bytes: bytes, output_dir, user_id: int,
    ...
    path = base / f"selfie_{user_id}_{stamp}_{trigger_source}.png"
...
def luvia_selfie(
    ...
    conn = store.connect()
    try:
        # 3. Quota — checked before spending a backend call.
        ...
        # 4. Edit the reference with the scene.
        ...
        # 5-7. Save the bytes, log the (now actually produced) selfie, return the path.
